...
window.onload = function () {
   ...
   =============================== */
    ...
    const selectedValue = select.querySelector(".selected_value_ar");
    const report_select_value_ar = document.getElementById("report_select_value_ar");
    ...
    optionsList.addEventListener("click", (e) => {
        ...
        if (!option) return;
        selectedValue.textContent = option.textContent;
        optionsList.querySelectorAll("li").forEach((o) => o.classList.remove("active_ar"));
        option.classList.add("active_ar");
        dropdown.style.display = "none";
        report_select_value_ar.dataset.id = option.getAttribute("value");
    });

    search.addEventListener("keyup", () => filterOptions(search.value.toLowerCase()));
    ...
    const optionsList = select.querySelector(".options_ar"); // the <ul>
    const selectedValue = select.querySelector(".selected_value_ar");
    const bsw_selected_value_ar = document.getElementById("bsw_selected_value_ar");
...
function agency_custom_select_ar_by_bsw() {
    document.querySelectorAll("#agency_custom_select_ar").forEach((select) => {
        const agency_selected_value_ar = document.getElementById("agency_selected_value_ar");
        const multi_dropdown = select.querySelector(".multi_dropdown_ar");
        ...
        newOptionsList.addEventListener("click", (e) => {
            ...
            if (!option) return;
            selectedValue.textContent = option.textContent;
            newOptionsList.querySelectorAll("li").forEach((o) => o.classList.remove("active_ar"));
            option.classList.add("active_ar");
            ...
            let new_agency_selected_value_ar = document.getElementById("agency_selected_value_ar");
             new_agency_selected_value_ar.dataset.id = agencyId;

            // Fetch CLP types for the selected agency
            const clpUrl = commonTicketUrl +
                `web_fontainer_ticket_api/clp_type.php?agency_type=${encodeURIComponent(agencyId)}`;
            fetch(clpUrl, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${commonTicketBearerToken}`,
        ...
        newSearch.addEventListener("keyup", () => filterOptions(newSearch.value.toLowerCase()));
...
function clp_custom_select_ar_by_agency() {
    document.querySelectorAll(".multi_select_ar").forEach((select) => {
        ...
        tagsBox.innerHTML = '<span class="multi_placeholder_ar">Select type of CLP</span>';
        ...
        newSearch.addEventListener("keyup", () => filter(newSearch.value.toLowerCase()));

        function filter(val) {
            newOptionsList.querySelectorAll("li").forEach((o) => {
                const oVal = o.dataset.value || o.getAttribute("value");
                if (values.includes(oVal)) { o.style.display = "none"; return; }
...
function closeChatWhenEventOccurs(){
    ...
    localStorage.removeItem("token");
    localStorage.removeItem("time");
    let ticketsTabOnClose1 = document.querySelector('.tab_ar[data-tab="tickets_ar"]');
    if (ticketsTabOnClose1) {
        document.querySelectorAll(".tab_ar").forEach(t => t.classList.remove("active_ar"));
        ticketsTabOnClose1.classList.add("active_ar");
        document.querySelectorAll(".tab_content_ar").forEach(c => c.classList.remove("active_ar", "fade_in"));
        ...
        data.data.forEach(item => {
            ...
            if (item.receive_status === 1) {
                const html =
                    '<div class="user_msg_ar">' +
                    '<span class="username_ar">You</span>' +
                    '<div class="chat_message_ar user_ar">' + item.message_body + '</div>' +
                    '<span class="reply_time_live_chat_user_ar">' + getCurrentTime() + '</span>' +
                    '</div>';
            ...
            if (item.receive_status === 2) {
                ...
                if (item.conversation_type_id === 3) {
                    const html =
                        '<div class="agent_msg_ar">' +
                        '<span class="agentname_ar">Operator</span>' +
                        '<div class="chat_message_ar bot_ar">' + item.message_body + '</div>' +
                        '<span class="reply_time_live_chat_agent_ar">' + getCurrentTime() + '</span>' +
                        '</div>';
                    liveContainer.insertAdjacentHTML("beforeend", html);

                // FAQ / Product bot reply
                } else {
                    const replyText = (item.reply && item.reply.reply) ? item.reply.reply : item.message_body;
                    const file      = item.reply && item.reply.file;

                    let html =
                        '<div class="agent_msg_ar">' +
                        '<span class="agentname_ar">Operator</span>' +
                        '<div class="chat_message_ar bot_ar">' + replyText + '</div>';

                    if (file) {
                        html += '<img src="' + file + '" class="image_text_ar">';
                    }
