...
/**
 * _get_mobile_number_13 method return date from datetime
 * @param string $number
 * @return string
 * @author Md. Amzad Hossain Jacky <amzadhossainjacky@gmail.com>
 */
if (!function_exists('_get_mobile_number_13')) {
    function _get_mobile_number_13(string $number)
    {
        return "88{$number}";
    }
}
 ...
 * @author Md. Amzad Hossain Jacky <amzadhossainjacky@gmail.com>
 */
if (!function_exists('_get_generate_token')) {
    function _get_generate_token()
    {
        $timestamp = time(); // Get current timestamp in seconds
        $bytes = random_bytes(32 - strlen($timestamp)); // Adjust length for timestamp
        $token = $timestamp . bin2hex($bytes);
...
if (!function_exists('_time_format')) {
    function _time_format($datetime, $format = "H:i:s")
    {
        $time = optional($datetime->created_at)->format($format);
        return $time;
    }
}

if (!function_exists('_time_format_changes')) {
    function _time_format_changes($datetime)
    {
       
            $dateObject = new DateTime($datetime);
            
            // Format the date to 12-hour format with leading zeros
            $formattedTime = $dateObject->format('h:i A'); // 'h' for 12-hour format, 'i' for minutes, 'A' for AM/PM
